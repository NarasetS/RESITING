...
def process_farm_area_suitability():
    ...
    # Check for empirical weights generated by the Relative Importance Framework
    weights = default_weights.copy()
    empirical_weights_path = Path("weightfactors/empirical_weights.json")
    
    if empirical_weights_path.exists():
        print(f"Loading empirical data-driven weights from {empirical_weights_path}...")
        try:
            with open(empirical_weights_path, 'r') as f:
                emp_weights = json.load(f)
                # Update technologies that have valid empirical models
                for tech in all_techs:
                    tech_weights = normalize_weight_values(emp_weights.get(tech))
                    if len(tech_weights) == len(criteria_names):
                        weights[tech] = tech_weights
                        print(f"  -> Applied empirical weights for {tech}")
                        print_weight_summary(tech, weights[tech])
        except Exception as e:
            print(f"  -> Failed to load empirical weights ({e}). Falling back to defaults.")
    else:
        print("No empirical weights found. Using default heuristic weights.")
